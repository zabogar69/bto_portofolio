...
function object_select_tag($object, $method, $options = array(), $default_value = null)
{
  ...
  $peer_method_parameter = _get_option($options, 'peer_method_parameter');

  $text_method = _get_option($options, 'text_method');

  $key_method = _get_option($options, 'key_method', 'getPrimaryKey');

  $obj = _getObj($related_class, $peer_method, $peer_method_parameter);  
  
  $select_options = _get_options_from_objects($obj, $text_method, $key_method);
...
function _getObj($related_class, $peer_method, $peer_method_parameter=null)
{
	$peerClassName = $related_class."Peer";	
	if($peer_method_parameter)
		$objects = call_user_func($peerClassName .'::'.$peer_method, $peer_method_parameter);
	else
		$objects = call_user_func($peerClassName .'::'.$peer_method);
