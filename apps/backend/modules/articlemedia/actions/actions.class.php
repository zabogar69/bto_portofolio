 ...
 */
class articlemediaActions extends taActionsBackend
{
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
	...
	public function executeMediaByTokenAndArticleId(sfWebRequest $request)
	{
		...
		{
			$this->items = ArticleMediaPeer::retrieveMediasByArticleId($request->getParameter("id"));	
		}
  ...
  public function executeAjaxDelete(sfWebRequest $request)
  {
		...
		{
			$Articlemedia = ArticleMediaPeer::retrieveByMediaId($request->getParameter('mediaId'));
			$Media = MediaPeer::retrieveByPk($request->getParameter('mediaId'));
			
			try
			{
				if($Articlemedia) $Articlemedia->delete();
				$Media->delete();
				$json['success'] = 'true';
				$json['message'] = '';
			}
