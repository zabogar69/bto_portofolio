	...
	/* ]]> */
    	...
    	<div class="sf_admin_list">
		    <table width="100%" cellspacing="0" cellpadding="0">
		      <caption><?php echo __('Media List') ?></caption>
			  <thead>
		      	<tr>
		      		<td colspan="9">
			      		<table cellspacing="0" cellpadding="0" width="100%">
					  		<tr>
				  				<td align="left">
					      			<form action="<?php echo url_for('media_collection', array('action' => 'filter')) ?>" method="post" id="filterForm">
										...
										<?php echo $filters['mediacategory_id']->renderLabel(__('Category')) ?>
							            <?php echo $filters['mediacategory_id']->render(array('style' => 'width: 150px;')) ?>
							            <?php echo $filters['mediacategory_id']->renderError() ?>
										<?php echo link_to(__('Reset', array(), 'sf_admin'), 'media_collection', array('action' => 'filter'), array('query_string' => '_reset', 'method' => 'post')) ?>
							            <input type="submit" value="<?php echo __('Filter', array(), 'sf_admin') ?>" />
					      			</form>
				      			</td>
					      		<td align="right">
								  <?php echo button_to(__('Multiple Upload', array(), 'sf_admin'), 'media/multiple')?>
								  <?php echo button_to(__('New', array(), 'sf_admin'), '@media_new')?>
		      ...
		      <tfoot>
		        <tr>
		          <th colspan="9">
		            <?php echo $filters->getMaxPerPageHtml(__("Display"), url_for('@media'), $displayNumber) ?> 
					<?php if ($pager->haveToPaginate()): ?>
		              <div class="sf_admin_pagination">
						  <a href="<?php echo url_for('@media') ?>?page=1">
						  ...
						  <a href="<?php echo url_for('@media') ?>?page=<?php echo $pager->getPreviousPage() ?>">
						  ...
						  <?php foreach ($pager->getLinks() as $page): ?>
						    <?php if ($page == $pager->getPage()): ?>
						      <?php echo $page ?>
						    <?php else: ?>
						      <a href="<?php echo url_for('@media') ?>?page=<?php echo $page ?>"><?php echo $page ?></a>
						    <?php endif; ?>
						  <?php endforeach; ?>
						
						  <a href="<?php echo url_for('@media') ?>?page=<?php echo $pager->getNextPage() ?>">
						  ...
						  <a href="<?php echo url_for('@media') ?>?page=<?php echo $pager->getLastPage() ?>">
		      ...
		      <tbody id="table_order">
				...
				<?php endif; ?>
		  		<tr>
					...
					<th class="hdr idCol">
					  <?php if ('id' == $sort[0]): ?>
					    <?php echo link_to(__('Id'), '@media', array('query_string' => 'sort=id&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
					    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
					  <?php else: ?>
					    <?php echo link_to(__('Id'), '@media', array('query_string' => 'sort=id&sort_type=asc')) ?>
					  <?php endif; ?>
					</th>
					<th class="hdr actCol">
					  <?php if ('state' == $sort[0]): ?>
					    <?php echo link_to(__('State'), '@media', array('query_string' => 'sort=state&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
					    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
					  <?php else: ?>
					    <?php echo link_to(__('State'), '@media', array('query_string' => 'sort=state&sort_type=asc')) ?>
					  <?php endif; ?>
					</th>
					<th class="hdr">
					  <?php if ('title' == $sort[0]): ?>
					    <?php echo link_to(__('Title'), '@media', array('query_string' => 'sort=title&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
					    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
					  <?php else: ?>
					    <?php echo link_to(__('Title'), '@media', array('query_string' => 'sort=title&sort_type=asc')) ?>
					  <?php endif; ?>
					</th>
					<th class="hdr">
					...
					<th class="hdr">
					  <?php if ('mimetype' == $sort[0]): ?>
					    <?php echo link_to(__('Type'), '@media', array('query_string' => 'sort=mimetype&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
					    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
					  <?php else: ?>
					    <?php echo link_to(__('Type'), '@media', array('query_string' => 'sort=mimetype&sort_type=asc')) ?>
					  <?php endif; ?>
					</th>
					<th class="hdr">
					  <?php if ('mediacategory_title' == $sort[0]): ?>
					    <?php echo link_to(__('Category'), '@media', array('query_string' => 'sort=mediacategory_title&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
					    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
					  <?php else: ?>
					    <?php echo link_to(__('Category'), '@media', array('query_string' => 'sort=mediacategory_title&sort_type=asc')) ?>
					  <?php endif; ?>
					</th>
					<th class="hdr">
					    <?php echo __('Author') ?>
		        ...
		        <?php foreach ($pager->getResults() as $i => $item): ?>
		          <tr class="highlight" id="<?php echo "item_".$item->getId() ?>">
					...
					<td class="tdata">
					  <?php echo link_to($item->getStateHtml(), 'media/publish?id='.$item->getId().'&publish='.($item->getState()=='A'?'0':'1'))?>
    				</td>
					<td class="tdata">
					...
					<td class="tdata" align="center">
					  <?php 
						if($item->hasSibling())
							echo link_to(image_tag('arrowup.gif'), 'media_sortup', $item);
						else
					  ...
					  <?php 
						if($item->hasSibling(false))
							echo link_to(image_tag('arrowdown.gif'), 'media_sortdown', $item);
						else
							echo image_tag('spacer2.gif');
					  ?>
				  	</td>
					...
					<td class="tdata" style="white-space: nowrap;">
		            	<?php echo link_to(__('Edit', array(), 'sf_admin'), 'media_edit', $item) ?>
    					|
    					<?php echo link_to(__('Delete', array(), 'sf_admin'), 'media_delete', $item, array('method' => 'delete', 'confirm' => __('Are you sure?')))?>
		            </td>
		          </tr>
		        <?php endforeach; ?>
		        </form>
