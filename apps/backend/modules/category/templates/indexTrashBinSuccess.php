  ...
  <div id="container">
	...
	<div class="sf_admin_list">
	    <table width="100%" cellspacing="0" cellpadding="0">
	      <caption><?php echo __('Category Trash Bin') ?></caption>
		  <thead>
	      	<tr>
	      ...
	      <tfoot>
	        <tr>
	          <th colspan="6">
	            <?php echo $filters->getMaxPerPageHtml(__("Display"), url_for('category/indexTrashBin'), $displayNumber) ?> 
	            <?php if ($pager->haveToPaginate()): ?>
	              <div class="sf_admin_pagination">
					  <a href="<?php echo url_for('category/indexTrashBin') ?>?page=1">
					  ...
					  <a href="<?php echo url_for('category/indexTrashBin') ?>?page=<?php echo $pager->getPreviousPage() ?>">
					  ...
					  <?php foreach ($pager->getLinks() as $page): ?>
					    <?php if ($page == $pager->getPage()): ?>
					      <?php echo $page ?>
					    <?php else: ?>
					      <a href="<?php echo url_for('category/indexTrashBin') ?>?page=<?php echo $page ?>"><?php echo $page ?></a>
					    <?php endif; ?>
					  <?php endforeach; ?>
					
					  <a href="<?php echo url_for('category/indexTrashBin') ?>?page=<?php echo $pager->getNextPage() ?>">
					  ...
					  <a href="<?php echo url_for('category/indexTrashBin') ?>?page=<?php echo $pager->getLastPage() ?>">
	      ...
	      <tbody id="table_order">
			...
			<tr>
				...
				<th class="hdr idCol">
				  <?php if ('id' == $sort[0]): ?>
				    <?php echo link_to(__('Id'), 'category/indexTrashBin', array('query_string' => 'sort=id&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
				    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
				  <?php else: ?>
				    <?php echo link_to(__('Id'), 'category/indexTrashBin', array('query_string' => 'sort=id&sort_type=asc')) ?>
				  <?php endif; ?>
				</th>
				<th class="hdr">
				  <?php if ('title' == $sort[0]): ?>
				    <?php echo link_to(__('Title'), 'category/indexTrashBin', array('query_string' => 'sort=title&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
				    <?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
				  <?php else: ?>
				    <?php echo link_to(__('Title'), 'category/indexTrashBin', array('query_string' => 'sort=title&sort_type=asc')) ?>
				  <?php endif; ?>
				</th>
				<th class="hdr">
	        ...
	        <?php foreach ($pager->getResults() as $i => $item): ?>
	          <tr class="highlight" id="<?php echo "item_".$item->getId() ?>">
				...
				<td class="tdata" style="white-space: nowrap;">
	            	<?php echo link_to(__('Restore'), 'category_restore', $item) ?>
					|
					<?php echo link_to(__('Destroy'), 'category_destroy', $item, array('confirm' => __('Are you sure?')))?>
	            </td>
	          </tr>
	        <?php endforeach; ?>
			  </form>
