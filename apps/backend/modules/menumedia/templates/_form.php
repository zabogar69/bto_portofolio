...
<body style="background-image:none">
	<div id="mf"><iframe name="media_frame" style="display:none"></iframe>	
		<div id="mf-form">
			<form id="media-upload" target="media_frame" action="<?php echo url_for('menumedia/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" enctype="multipart/form-data">
			<?php if (!$form->getObject()->isNew()): ?><input type="hidden" name="sf_method" value="put" /><?php endif; ?>
			<?php echo $form->renderHiddenFields(false) ?>
			<div class="mf-form"><?php echo __("Voeg uw foto's  toe die u bij dit project getoond wilt hebben") ?></div>
			<div class="mf-form-info"><?php echo __('Maximale grote: 10 MB, toegestane file types: jpg, png, gif')?></div>
