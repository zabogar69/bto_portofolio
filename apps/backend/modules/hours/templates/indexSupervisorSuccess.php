<h1>(Supervisor) Uren schrijven - <strong>Week <?php echo $currentWeek; ?> - <?php echo $projId > 0? $projItem->getName():"";?> </strong></h1>
					<hr>
					...
					<form id="select-company" action="<?php echo url_for("hours/saveSupervisor");?>" method="post">
						<div class="formrow">
							...
							<script>
								$(document).ready(function() {
									$("#select-project").change(function() {
										...
										window.location = "<?php echo url_for("hours/indexSupervisor")?>?w=<?php echo $currentWeek;?>&p="+ curVal;
							...
							<script>
								$(document).ready(function() {
									$("#select-user").change(function() {
										...
										window.location = "<?php echo url_for("hours/indexSupervisor")?>?w=<?php echo $currentWeek;?>&p=<?php echo $projId; ?>&u="+ curVal;
						...
						<p class="paging"><a class="prev" href="<?php echo url_for("hours/indexSupervisor?w=".($currentWeek-1)."&p=".$projId."&u=".$userId);?>"><span class="icon">Å</span></a> <a class="next" href="<?php echo url_for("hours/indexSupervisor?w=".($currentWeek+1)."&p=".$projId."&u=".$userId);?>"><span class="icon">Ä</span></a>Week <?php echo $currentWeek; ?></p>
						<table id="time" class="tablesorter"> 
							...
							<?php foreach($projItem->getComponents() as $component):?>
								<tr> 
									<td><?php echo $component->getName()?>&nbsp;<?php echo $component->getState()!= "A"?"<font color='red'>(inactief)</font>":""?></td>
									
									<?php for($i=0;$i<=5;$i++):?>
										<?php $cdate =  date('Y-m-d', strtotime('+'.$i.' days', strtotime($weekDate1))); ?>
										<td>
											<select class="day<?php echo $i;?>" name="hours[<?php echo $component->getId();?>_<?php echo $cdate;?>]">
											<?php 
